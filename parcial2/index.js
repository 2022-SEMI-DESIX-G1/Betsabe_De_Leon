...
((Utils) => {
  const App = {
    htmlElements: {
      ...
      hidden: `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      baby:`<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    init: () => {
      ...
      App.htmlElements.pokemonClean.addEventListener(
        "click",App.handlers.pokemonCleanOnSubmit);
      ...
      pokemonFinderFormOnSubmit: async (e) => {
        ...
        try {
         ...
         if (searchType === "ability") {
              ...
              App.htmlElements.evolutions = App.evolution.getEvolutionResponse(allEvolution);

              App.htmlElements.evolutions.unshift(chain);
            ...
            const toEvo = await Utils.getEvolution(response.species.url);

            const toEvo2 = await Utils.getEvolution(toEvo.evolution_chain.url);
    ...
    evolution: {
      ...
      getEvolutionResponse: (evolutions) => {
    ...
    templates: {
      ...
      pokemonCard: ({ id, name, weight, height, sprites,  abilities, chain}) => {
        ...
        `<ul>
          ...
          <li>${ability.name}
              ...
              ${is_hidden? App.htmlElements.hidden: ""}</li>
        ...
        var evolutionList = evolution.map(
          (element) => `<li>${element.species.name} ${element.is_baby ? App.htmlElements.baby : ""}</li> `

        );

      
        return `<div id="pokemon">
                <h1 class="name">${name} (${id}) </h1>
                <div class="firstElements">
                  <div class="info">
                  <h3 class="titles">Sprites</h3>
                  <div class="imgs">
                        <img class=pika src=${sprites.front_default}></img>
                        <img class=pika1 src=${sprites.back_default}></img>
                  </div>
                  </div>
                ...
                <div class="info">
                    <h3 class="tit">Evoluciones</h3>
                 <ul class="evolution">${evolutionList.join("")}</ul>
                 </div>

                 <div class="info">
                 <h3 class="tit">Habilidades</h3>
                 <ul class="habilidad">${pokemonList.join("")}</ul>
                 </div>
                 </div>
                 </div>
                 </form>`;
      ...
      abilityCard: ({ id, name, pokemon }) => {
        ...
        let cadena = name.charAt().toUpperCase();
        let cadena2 = name.slice(1);
        return `<div class="ability">
                <h1 class=nombre>${cadena + cadena2}</h1>
                <p class=whocan>¿Quien puede aprenderlo?</p>
                <ul class=list>${pokemonList.join("")}</ul
                </div>` 
      },
    },
  };
